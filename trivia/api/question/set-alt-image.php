...
if(!empty($data->id)){
    ...
    if(!empty($question->question)) {
        ...
        $updatedQuestion->is_alt_img = (!empty($data->isAltImage)) ? 1 : 0;
        
        // $response = $updatedQuestion->setAltImage();
        if($updatedQuestion->setAltImage())
        {
            // set response code - 201 created
            http_response_code(200);
            echo json_encode(array(
                "status" => (!empty($updatedQuestion->is_alt_img)) ? $updatedQuestion->is_alt_img : 0,
